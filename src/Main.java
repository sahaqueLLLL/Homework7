...
public class Main {
    ...
    public static List<String> methodOne(List<String> names) {
       return names.stream().filter(s -> s.contains("King")).toList();
    ...
    public static List<String> methodFour(List<String> names) {
        return names.stream().map(n -> n.split(" ")[1]).filter(n -> n.startsWith("W")).sorted().distinct().toList();
    ...
    public static Integer methodFive(List<String> names) {
        return (int)names.stream().map(n -> n.split(" ")[0]).filter(n -> n.length() > 6).distinct().count();
    }
    ...
    public static List<String> methodSeven(List<String> names) {
        return  names.stream().map(n -> n.split(" ")).filter(s -> s[1].startsWith("K") || s[1].startsWith("S")).
                map(s -> s[0] + " " + s[1].charAt(0) + ".").distinct().sorted().toList();
